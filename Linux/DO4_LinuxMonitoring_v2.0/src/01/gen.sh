...
then
    ...
    for (( i=1; i<$caunter_gen; i++ )) do
        ...
        then
            ...
            count_sim_2=1
            count_sim_1=1
        elif [ $size_sim -gt 4 ] && [ $count_sim_4 -gt $count_sim_5 ];
        then
            let count_sim_5++
            count_sim_4=1
            count_sim_3=1
    ...
    for (( i=0; i<$( expr $count_sim_1 + $count_sim_2 + $count_sim_3 + $count_sim_4 + $count_sim_5 + $count_sim_6 + $count_sim_7 ); i++ )) do
        ...
        then
            name+="${name_sim:3:1}"
        elif [ $size_sim -gt 4 ] && [ $i -lt $( expr $count_sim_1 + $count_sim_2 + $count_sim_3 \
             + $count_sim_4 + $count_sim_5 ) ];
        then
            name+="${name_sim:4:1}"
        elif [ $size_sim -gt 5 ] && [ $i -lt $( expr $count_sim_1 + $count_sim_2 + $count_sim_3 \
             + $count_sim_4 + $count_sim_5 + $count_sim_6 ) ];
        then
            name+="${name_sim:5:1}"
        elif [ $size_sim -eq 7 ];
        then
