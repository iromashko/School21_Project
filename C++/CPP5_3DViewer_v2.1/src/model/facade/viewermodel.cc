...
void ViewerModel::LoadObjModelToScene(const std::string &path, Scene *scene) {
  ...
  if (current_index == -1) {
    ...
    ss.stop();
    qDebug() << ss.seconds();
    object_data = result.first;
    Normalizer::GetNormalizer().Normalize(object_data, 1.0f);
    AddModelToHistory(path);
    is_loadable = result.second;
  } else {
    ss.start();
    object_data = object_data_history[current_index].second;
    ss.stop();
    qDebug() << ss.seconds();
  }

  if (is_loadable) {
    scene->LoadObjectModel(object_data.GetVertexes(), object_data.GetIndexes());
  }
}
...
void ViewerModel::ChangeSceneRotate(Scene *scene, float angle, bool x, bool y,
                                    bool z) {
  if (x) {
    TransformModel::GetTransform().Rotate(object_data, angle, x);
  } else if (y) {
    TransformModel::GetTransform().Rotate(object_data, angle, x, y);
  } else if (z) {
    TransformModel::GetTransform().Rotate(object_data, angle, x, y, z);
  }
  scene->LoadObjectModel(object_data.GetVertexes(), object_data.GetIndexes());
}
...
void ViewerModel::ChangeSceneScale(Scene *scene, float scale) {
  if (scale >= 1e-6) {
    TransformModel::GetTransform().Scale(object_data, scale);
    scene->LoadObjectModel(object_data.GetVertexes(), object_data.GetIndexes());
  }
}
...
void ViewerModel::ChangeSceneMove(Scene *scene, float move, bool x, bool y,
                                  bool z) {
  if (x) {
    TransformModel::GetTransform().Move(object_data, move / 75, x);
  } else if (y) {
    TransformModel::GetTransform().Move(object_data, move / 75, x, y);
  } else if (z) {
    TransformModel::GetTransform().Move(object_data, move / 75, x, y, z);
  }
  scene->LoadObjectModel(object_data.GetVertexes(), object_data.GetIndexes());
}
...
void ViewerModel::MakeImage(Scene *scene) {
  RecorderContext *record_image = new RecorderContext(new RecorderImage);
  record_image->RecordObject(scene, s21::TypeRecorder::Image);
}

void ViewerModel::MakeGif(Scene *scene) {
  RecorderContext *record_gif =
      new RecorderContext(new RecorderGif(object_data));
  record_gif->RecordObject(scene, s21::TypeRecorder::Gif);
}

void ViewerModel::MakeGifRotate(Scene *scene) {
  RecorderContext *record_gif =
      new RecorderContext(new RecorderGif(object_data));
  record_gif->RecordObject(scene, s21::TypeRecorder::GifRotation);
}
...
void ViewerModel::AddModelToHistory(std::string path) {
  if (GetModelIndexFromHistory(path) == -1) {
    object_data_history.push_back(
        std::make_pair(path.substr(path.find_last_of("/\\") + 1), object_data));
  }
}
...
int ViewerModel::GetModelIndexFromHistory(std::string path) {
  ...
  for (std::size_t i = 0; i != object_data_history.size(); ++i)
    if (object_data_history[i].first == filename) return i;
  return -1;
}
